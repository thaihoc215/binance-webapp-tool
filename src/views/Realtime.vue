<template>
  <div class="realtime-page">
    ...
    <div class="text-center">
      ...
      <div>
        Total PNL: <span>{{ totalPNL | currency }}</span>
      </div>
      <div>
        Total PNLVN: <span>{{ totalPNLVN | currency }}</span>
      </div>
  ...
  methods: {
    ...
    onStream(res) {
      ...
      this.slots = this.slots.map((x) => {
        ...
        currentTotal += item.PNL;
        return item;
      });
      this.totalPNL = currentTotal;
      this.totalPNLVN = currentTotal * 23000;
    },
